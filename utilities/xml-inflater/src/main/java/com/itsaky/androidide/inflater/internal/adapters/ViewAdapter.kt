...
open class ViewAdapter<T : View> : IViewAdapter<T>() {
    ...
    override fun createAttrHandlers(create: (String, AttributeHandlerScope<T>.() -> Unit) -> Unit) {
        ...
        create("paddingStart") {
            view.updatePaddingRelative(start = parseDimension(context, value))
        }
    ...
    protected open fun applyRelativeLayoutParams(
        ...
        when (name) {
            ...
            "layout_alignParentBottom" ->
                setRuleIf(parseBoolean(value), ALIGN_PARENT_BOTTOM, params)
    ...
    private fun setRuleIf(condition: Boolean, rule: Int, params: RelativeLayout.LayoutParams) {
        if (condition) {
            params.addRule(rule)
        } else {
