...
protected constructor(protected open val parser: XmlPullParser?, open var minDepth: Int) :
     ...
     * if the drawable parser keeps looking for [XmlPullParser.START_TAG] or
     * [ ][XmlPullParser.END_TAG].
     *
     * The [LayerListParser] can contain tags which are expected to be parsed by other parsers. So,
     * if the nesting parser does not check if this method if `true`, it might consume all the
     * events and the [LayerListParser] will have invalid data to parse. In some cases, the nested
     * parser might consume all the events until [XmlPullParser.END_DOCUMENT].
     *
