...
class ShapeDrawableParser protected constructor(parser: XmlPullParser?, minDepth: Int) :
    ...
    private fun parseStroke(context: Context, drawable: GradientDrawable) {
        ...
        if (index != -1) {
            drawable.setStroke(
                strokeWidth,
                parseColor(context, value(index)).also { strokeColor = it },
            )
        }
    ...
    private fun parseGradient(context: Context, drawable: GradientDrawable) {
        ...
        drawable.orientation =
            if (index == -1) LEFT_RIGHT else parseGradientOrientation(value(index))
        ...
        drawable.gradientRadius =
            if (index == -1) drawable.gradientRadius else value(index).toFloat()
    ...
    private fun parseCorners(context: Context, drawable: GradientDrawable) {
        ...
        drawable.cornerRadius =
            if (index == -1) 0f else parseDimension(context, value(index)).toFloat()
        var changed = false
