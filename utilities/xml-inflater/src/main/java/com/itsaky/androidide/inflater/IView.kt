...
interface IView {
     ...
     * @param apply Whether the attribute should be applied to the attribute. If this `false` then
     *   the attribute will be simply added to the attributes list.
    ...
    /**
     * Applies the given attribute without adding it to the attributes list or notifying the
     * attribute change listeners. Not resetting the attribute to its original value may have
     * unexpected result.
     *
     * @param attribute The attribute to apply.
     */
     ...
     * @return `true` if this view has an attribute entry with the given name namespace uri and
     ...
     * [android.view.MotionEvent.ACTION_UP] motion event. In case of an [IViewGroup], the
     * highlighted state is also changed when the view group receives the drag events.
     *
