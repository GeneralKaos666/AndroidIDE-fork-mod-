...
constructor(
    ...
    fun logMetrics() {
        if (
            !(metricsEnabled && DEBUG) || System.currentTimeMillis() - lastMetricsLog.get() < 1000
        ) {
            return
        }
     ...
     *
     * A [Recyclable] object implementation should prefer defining a default no-args constructor
     * which initializes the properties with the default values.
     */
    interface Recyclable {

        /**
         * Whether the object has been recycled. This property is supposed to be modified only by
         * the pool.
         */
        var isRecycled: Boolean

        /**
         * Recycle the this object. Implementations should make sure that all the properties are
         * resset to their default values (as if the object was newly created).
         */
        fun recycle()
    }
