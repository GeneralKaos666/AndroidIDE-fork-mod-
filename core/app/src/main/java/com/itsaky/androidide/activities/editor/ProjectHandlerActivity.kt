...
abstract class ProjectHandlerActivity : BaseEditorActivity() {
    ...
    private fun notifySyncNeeded(onConfirm: () -> Unit) {
        ...
        if (
            buildService == null || editorViewModel.isInitializing || buildService.isBuildInProgress
        )
            return
     ...
     *
     * @param buildVariantsProvider A function which returns the map of project paths to the
     *   selected build variants. This function is called asynchronously.
     */
    ...
    protected open fun onProjectInitialized(result: InitializeResult) {
        val manager = ProjectManagerImpl.getInstance()
        if (
            isFromSavedInstance && manager.projectInitialized && result == manager.cachedInitResult
        ) {
    ...
    protected open fun createFindInProjectDialog(moduleDirs: List<File>): AlertDialog? {
        ...
        builder.setPositiveButton(string.menu_find) { dialog, _ ->
            ...
            if (searchDirs.isEmpty()) {
                ...
                RecursiveFileSearcher.searchRecursiveAsync(text, extensionList, searchDirs) {
                    results ->
