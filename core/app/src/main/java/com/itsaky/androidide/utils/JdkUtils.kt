...
object JdkUtils {
    ...
    fun findJavaInstallations(): List<JdkDistribution> {
        ...
        // TODO: Find a way to efficiently list all JDK installations, including those which are
        // located
     ...
     * @return The [JdkDistribution] instance, or `null` if there was an error while getting
     *   required information from the installation.
     */
    @JvmStatic
    ...
    internal fun readDistFromProps(properties: String): JdkDistribution? {
        val javaHome =
            Regex("java\\.home\\s*=\\s*(.*)").find(properties)?.groupValues?.get(1)
                ?: run {
                    log.error(
                        "Failed to determine property 'java.home'. Properties: {}",
                        properties,
                    )
                    return null
                }
        ...
        val javaVersion =
            Regex("java\\.version\\s*=\\s*(.*)").find(properties)?.groupValues?.get(1)
                ?: run {
                    log.error(
                        "Failed to determine property 'java.version'. Properties: {}",
                        properties,
                    )
                    return null
                }
