...
class QuickRunWithCancellationAction(context: Context, override val order: Int) :
    ...
    override fun createColorFilter(data: ActionData): ColorFilter? {
        return data.getContext()?.let {
            PorterDuffColorFilter(
                it.resolveAttr(
                    if (data.isBuildInProgress()) R.attr.colorError else R.attr.colorSuccess
                ),
    ...
    private fun onModuleSelected(
        ...
        actionScope
            .launch(Dispatchers.Default) {
                ...
                val result =
                    withContext(Dispatchers.IO) { buildService.executeTasks(taskName).get() }
    ...
    private fun ActionData.isBuildInProgress(): Boolean {
        ...
        return context?.editorViewModel?.let { it.isInitializing || it.isBuildInProgress } ==
