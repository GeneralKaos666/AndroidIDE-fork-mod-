...
class CodeEditorView(context: Context, file: File, selection: Range) :
    ...
    /**
     * The [CoroutineContext][kotlin.coroutines.CoroutineContext] used to reading and writing the
     * file in this editor. We use a separate, single-threaded context assuming that the file will
     * be either read from or written to at a time, but not both. If in future we add support for
     ...
     *   `false`, it means that there was an error saving the file or the content of the file was
     *   not modified and hence the save operation was skipped.
     */
        ...
        withContext(Dispatchers.Main.immediate) {
            withEditingDisabled {
                withContext(readWriteContext) {
                    ...
                    // the writeTo function acquires lock to the Content object before writing and
                    // releases
                    // the lock after writing
                    // if there are any suspend function calls in between, the lock and unlock calls
                    // might not
    ...
    private fun initializeContent(content: Content, file: File, selection: Range) {
        ...
        ideEditor.postInLifecycle {
            ...
            // but in this case, file is read from disk and hence the contents are not modified at
            // all
