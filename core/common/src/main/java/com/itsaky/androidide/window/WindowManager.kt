...
class WindowManager(
    ...
    /**
     * Activity that was registered with this instance of [WindowManager] at creation. This is used
     * to find the token identifier of the window when requesting layout information from the
     ...
     * activities in multi-window mode, the metrics returned are based on the current bounds that
     * the user has selected for the [Activity][android.app.Activity]'s window.
     *
     ...
     * example, for activities in multi-window mode the metrics returned are based on what the
     ...
     * of the display are not available to windows created for the associated [Context]. For
     * example, devices with foldable displays that wrap around the enclosure may split the physical
     * display into different regions, one for the front and one for the back, each acting as
     * different logical displays. In this case [.getMaximumWindowMetrics] would return the region
     * describing the side of the device the associated [context&#39;s][Context] window is placed.
     *
