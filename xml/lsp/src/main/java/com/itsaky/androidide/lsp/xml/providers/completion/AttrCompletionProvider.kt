...
open class AttrCompletionProvider(provider: ICompletionProvider) :
    ...
    protected open fun completeForNamespace(
        ...
        if (namespace == null) {
            log.warn(
                "Namespace is null. Cannot compute completions for namespace prefix: {}.",
                nsPrefix,
            )
            return
        }
    ...
    protected open fun findNodeStyleables(
        ...
        val widgets =
            Lookup.getDefault().lookup(WidgetTable.COMPLETION_LOOKUP_KEY) ?: return emptySet()
        ...
        if (widget != null) {
            ...
            // If the developer follows the naming convention then only the completions will be
            // provided
