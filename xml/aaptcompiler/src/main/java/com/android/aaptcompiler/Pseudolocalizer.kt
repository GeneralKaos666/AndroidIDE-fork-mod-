    ...
    private fun processHtml(sourcePartial: StringBuilder, source: String, startIndex: Int): Int {
        ...
        htmlTag@ while (currentIndex < size) {
            ...
            if (currentIndex >= size) {
                // we didn't find the closing bracket, it wasn't valid HTML. Needs to be
                // pseudolocalized, so
                // return starting index, and append the first character to the partial string
                // builder.
            ...
            if (lastDepth != depth || pos == size - 1) {
                ...
                if (chunkSize.isTruthy()) {
                    ...
                    if (pseudo) {
                        ...
                        originalText[lastPos] == ARG_START && originalText[nextPos - 1] == ARG_END
                    ) {
