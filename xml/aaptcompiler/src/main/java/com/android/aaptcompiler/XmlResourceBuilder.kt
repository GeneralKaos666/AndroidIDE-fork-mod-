...
class XmlResourceBuilder(val file: ResourceFile, val skipWhitespaceText: Boolean = true) {
    ...
    fun findAttribute(name: String, namespace: String): String? {
        ...
        return attrList
            .find { it.getName() == name && it.getNamespaceUri() == namespace }
            ?.getValue()
    }
     ...
     *
     * The text is not immediately added, but is held by the builder so that consecutive text
     * elements can be accumulated as a single element for the protocol buffer. Whenever the current
     * XML tag is finished or a non-text element is encountered such as a comment or new tag, the
     * current text state is flushed, and the text element is added.
     */
