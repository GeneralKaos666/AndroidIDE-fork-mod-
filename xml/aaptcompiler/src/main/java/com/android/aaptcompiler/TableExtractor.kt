...
private class ParsedResource(
     ...
     * {@code this} is added. The connection of the resources to this resource should be reflected
     * in the [value] of the parsed resource.
     */
...
class TableExtractor(
    ...
    fun extract(inputFile: InputStream) {
        ...
        try {
            ...
            if (!documentStart.isStartDocument) {
                val userReadableSource =
                    logger.getOriginalSource(blameSource(source)).toString().trim()
     ...
     *
     * <p> The eventReader is assumed have just read the root "resources" start element. All
     * resource values extracted are added to the [table] property.
     *
     ...
     *   StartElement} element. After this method is invoked the eventReader will be placed after
     *   the corresponding end tag for element.
    ...
    private fun extractResource(
        ...
        // get name of the resource. This will be checked later, because not all xml elements
        // require
        // a name.
        ...
        if (resourceTypeName == "id") {
            ...
            when {
                ...
                item is Reference && item.id == null ->
                    // A null reference also means there is no inner element when ids are in the
                    // form:
                    //    <id name="name"/>
                    parsedResource.value = Id()
                (item is Reference && item.name.type != ID) || item !is Reference -> {
                    // if an inner element exists, the inner element must be a reference to another
                    // id
                    logError(
                        blameSource(parsedResource.source),
        ...
        if (canBeItem) {
            val (type, typeMask) =
                when (resourceTypeName) {
                    "bool" ->
                        Pair(AaptResourceType.BOOL, Resources.Attribute.FormatFlags.BOOLEAN_VALUE)
                    "color" ->
                        Pair(AaptResourceType.COLOR, Resources.Attribute.FormatFlags.COLOR_VALUE)
                    "configVarying" ->
                        Pair(
                            AaptResourceType.CONFIG_VARYING,
                            Resources.Attribute.FormatFlags.ANY_VALUE,
                        )
                    "dimen" ->
                        Pair(
                    ...
                    "integer" ->
                        Pair(
                            AaptResourceType.INTEGER,
                            Resources.Attribute.FormatFlags.INTEGER_VALUE,
                        )
                    "string" ->
                        Pair(AaptResourceType.STRING, Resources.Attribute.FormatFlags.STRING_VALUE)
        ...
        if (canBeBag) {
            ...
            if (parseBagMethod != null) {
                ...
                // Call the associated parse method. The type will be filled in by the parse
                // function
     ...
     *   failure.
     * @return The [Item] that represents the xml subtree. This will be {@code null} if the xml
     *   failed to be interpreted as a valid resource.
     */
    ...
    private fun parseEnumOrFlagItem(
        ...
        if (resValue == null) {
            logError(
                ...
                "Invalid value 'null' for <$tag>. Must be an integer.",
            )
            return null
        }
     ...
     *   [FlattenedXml.untranslatableSections] contains the sections of the string that should not
     *   be translated.
     */
    ...
    /**
     * parses the xml as a [AttributeResource] represented by the specified start element. Then
     * stores the value in the parsed resource.
     *
     * @param element The start element that represents the symbol to be parsed.
     * @param eventReader The xml to be read. The event reader should have just pulled the {@code
     *   StartElement} element. After this method is invoked the eventReader will be placed after
    ...
    /**
     * Parse the xml that is contained by the "array" tag. The valid format of the child items will
     * be parsed from the format attribute of {@code element}
     *
     ...
     *   the corresponding end tag for element.
     * @param parsedResource where the read resource will be placed.
     * @param resourceFormat A type mask that specifies which formats are valid for the child
     *   elements of the array to be interpreted as.
    ...
    private fun parseStyleImpl(
        ...
        if (parentAttribute != null) {
            // If the parent is empty, we don't have a parent but we don't attempt to infer one
            // either.
            if (parentAttribute.value.isNotEmpty()) {
                ...
                // Transform the namespace prefix to the actual package name, and mark the reference
                // as
    ...
    private fun parseDeclareStyleable(
        ...
        while (eventReader.hasNext()) {
            ...
            if (childName.namespaceURI.isEmpty() && childName.localPart == "attr") {
                ...
                // If this is a declaration, the package name may be in the name. Separate these
                // out.
    ...
    private fun parseOverlayable(
        ...
        if (
            actorAttribute != null && !actorAttribute.value.startsWith(Overlayable.ACTOR_SCHEME_URI)
        ) {
            logError(
                blameSource(source, element.location),
        ...
        while (eventReader.hasNext()) {
            ...
            when {
                childName.namespaceURI == XMLConstants.NULL_NS_URI &&
                    childName.localPart == "item" -> {
                    val childResource =
                        parseOverlayableItem(
                            childElement,
                            eventReader,
                            currentPolicies,
                            overlayable,
     ...
     *   StartElement} element. After this method is invoked the eventReader will be placed after
     *   the corresponding end tag for element.
     * @param policies The policies of the current policy block that this [OverlayableItem] is a
     *   part of. The value of policies should be non-zero.
    ...
    /**
     * parses the {@code element} as a PublicGroup resource. The {@code eventReader} will be after
     * the corresponding end of {@code element}
     */
