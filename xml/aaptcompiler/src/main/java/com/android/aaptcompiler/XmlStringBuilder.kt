...
class XmlStringBuilder(private val preserveSpaces: Boolean = false) {
    ...
    fun endSpan(): XmlStringBuilder {
        ...
        // When a span is ended, all state associated with whitespace truncation and quotation is
        // ended.
        resetTextState()
    ...
    fun startUntranslatable(): XmlStringBuilder {
        ...
        if (inUntranslatable) {
            error =
                "Attempt to nest untranslatable sections in flattened xml, which is not allowed."
            return this
        }
    ...
    fun endUntranslatable(): XmlStringBuilder {
        ...
        if (!inUntranslatable) {
            error =
                "Attempting to end untranslatable section while not in an untranslatable sections."
            return this
        }
    ...
    private fun getLastChar(text: String): Int {
        if (lastQuote == -1) {
            ...
            // If there were no characters, it's all whitespace, and the resulting string should be
            // empty.
            return 0
        }
