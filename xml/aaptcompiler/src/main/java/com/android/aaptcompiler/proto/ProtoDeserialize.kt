    ...
    val configDescription =
        ConfigDescription(
            ResTableConfig(
                ...
                screenLayout =
                    (screenLayoutDir.toInt() or
                            screenLayoutLong.toInt() or
                            screenLayoutSize.toInt())
                        .toByte(),
    ...
    for (untranslatable in pbMacro.untranslatableSectionsList) {
        untranslatableList.add(
            UntranslatableSection(
                untranslatable.startIndex.toInt(),
                untranslatable.endIndex.toInt(),
            )
        )
    }
    ...
    for (group in original.typeList) {
        ...
        for (entry in group.entryList) {
            ...
            if (entry.hasVisibility()) {
                ...
                resourceEntry.visibility =
                    Visibility(visibilitySource, visibilityComment, visibilityLevel)
            }
...
fun deserializeTableFromPb(
    ...
    for (resourcePackage in table.packageList) {
        if (
            !deserializePackageFromPb(resourcePackage, sourcePool, overlayables, logger, outTable)
        ) {
            return false
        }
    }
