    ...
    for (resourcePackage in table.packages) {
        ...
        if (packageId != null) {
            packageBuilder.setPackageId(
                Resources.PackageId.newBuilder().setId(packageId.toInt()).build()
            )
        }
                ...
                for (entry in idToEntries.values) {
                    ...
                    if (entryId != null) {
                        entryBuilder.setEntryId(
                            Resources.EntryId.newBuilder().setId(entryId.toInt()).build()
                        )
                    }
                    ...
                    if (entry.visibility.source.isNotEmpty()) {
                        visibilityBuilder.setSource(
                            serializeSourceToPb(entry.visibility.source, sourcePool)
                        )
                    }
                    ...
                    if (entryAllowNew != null) {
                        ...
                        if (entryAllowNew.source.isNotEmpty()) {
                            allowNewBuilder.setSource(
                                serializeSourceToPb(entryAllowNew.source, sourcePool)
                            )
                        }
                        entryBuilder.setAllowNew(
                            allowNewBuilder.setComment(entryAllowNew.comment).build()
                        )
                    }
                    ...
                    for (configValue in entry.values) {
                        entryBuilder.addConfigValue(
                            Resources.ConfigValue.newBuilder()
                                ...
                                .setValue(
                                    serializeValueToPb(configValue.value!!, sourcePool, logger)
                                )
                                .build()
                        )
                    }
...
fun serializeConfigToPb(
    ...
    configBuilder.setScreenRound(
        when (
            (config.screenLayout2.toInt() and ResTableConfig.SCREEN_LAYOUT2.SCREENROUND_MASK)
                .toByte()
        ) {
    ...
    configBuilder.setHdr(
        when ((config.colorMode.toInt() and ResTableConfig.COLOR_MODE.HDR_MASK).toByte()) {
            ResTableConfig.COLOR_MODE.HDR_YES ->
                ConfigurationOuterClass.Configuration.Hdr.HDR_HIGHDR
...
fun serializeOverlayableToPb(
    ...
    // Retrieve the index of the overlayable in the list of groups that have already been
    // serialized.
        ...
        ResValue.DataType.NULL -> {
            when (resVal.data) {
                ...
                ResValue.NullFormat.EMPTY ->
                    primitiveBuilder.setEmptyValue(
                        Resources.Primitive.EmptyType.newBuilder().build()
                    )
...
fun serializeValueToPb(value: Value, sourcePool: StringPool, logger: ILogger?): Resources.Value {
    ...
    if (value is Item) {
        ...
        when (value) {
            ...
            is ArrayResource ->
                compoundBuilder.setArray(serializeArrayToPb(value, sourcePool, logger))
