...
class ResourceTable(val validateResources: Boolean = false, val logger: BlameLogger? = null) :
    ...
    /**
     * Set of dynamic packages that this table may reference. Their package names get encoded into
     * the resources.arsc along with their compile-time assigned IDs.
     */
     ...
     *
     * @param name The full name of this resource. This includes package name, resource type, and
     ...
     * @param value The value associated with the given resource.
     * @return Returns false if and only if the resource was not able to be added. This is caused if
     *   the resource name failed to be validated, there exists a conflict with a resource already
     *   in the table, or the id is a dynamic id but does not match with the package, type, or
     *   entry.
     */
    ...
    /**
     * Same as [addFileReference], but doesn't verify the name of the file reference resource. This
     * is used when loading resources from an existing binary resource table that may have mangled
     * resources.
     *
     * @param name The full name of this resource. This includes package name, resource type and the
     *   entry name of the resource.
     ...
     * @return true, if and only if the visibility was able to be updated. This may not happen if
     *   the visibility has already been set to at least as visible as the new visibility.
     */
    ...
    /**
     * Attempts to find a package having the specified name and ID. If not found, a new package of
     * the specified parameters is created and returned.
     */
    ...
    private fun setOverlayableImpl(
        ...
        if (oldEntry != null) {
            val previousSource =
                logger?.getOriginalSource(blameSource(oldEntry.source))
    ...
    companion object {
        fun resolveValueCollision(existing: Value, incoming: Value): CollisionResult {
            ...
            // Attribute specific handling. Since declarations and definitions of attributes can
            // happen
            // almost anywhere, we need special handling to see which definition sticks.
            if (existingAttr.isCompatibleWith(incomingAttr)) {
                // The two attributes are both DECLs, but they are plain attributes with compatible
                // formats.
                // keep the strongest.
                return if (existingAttr.weak) CollisionResult.TAKE_NEW
                else CollisionResult.KEEP_ORIGINAL
            }

            if (
                existingAttr.weak &&
                    existingAttr.typeMask == Resources.Attribute.FormatFlags.ANY_VALUE
            ) {
            ...
            if (
                incomingAttr.weak &&
                    incomingAttr.typeMask == Resources.Attribute.FormatFlags.ANY_VALUE
            ) {
                // The incoming attribute may be a USE instead of a DECL. Keep the existing
                // attribute.
...
class ResourceGroup(val type: AaptResourceType) : IResourceGroup {
    ...
    internal fun getStyleable(
        entry: Map.Entry<String, SortedMap<Short?, ResourceEntry>>
    ): Styleable {
