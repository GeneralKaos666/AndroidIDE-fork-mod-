...
class ResourceTableTest {
    ...
    @Test
    fun testStyleables() {
        // Styleable children are nested deep under ResourceEntry, so the
        // ResourceGroup.getStyleable()
        ...
        Truth.assertThat(styleableEntryOne.value.values.first().name)
            .isEqualTo("styleable_parent_one")
        ...
        Truth.assertThat(styleableEntryTwo.value.values.first().name)
            .isEqualTo("styleable_parent_two")
    ...
    fun testAddMultipleResource() {
        ...
        val languageConfig =
            ConfigDescription(
                ResTableConfig(language = byteArrayOf('p'.code.toByte(), 'l'.code.toByte()))
            )
    ...
    fun testCompatibleDuplicateAttributesAreAllowed() {
        ...
        weak1.weak = true
        val weak2 = AttributeResource(FormatFlags.STRING_VALUE or FormatFlags.REFERENCE_VALUE)
        weak2.weak = true
    ...
    fun testProductVaryingValues() {
        ...
        val searchResult =
            table.findResource(parseResourceName("android:string/foo")!!.resourceName)
