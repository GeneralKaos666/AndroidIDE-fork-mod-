...
class PseudolocaleGeneratorTest {
    ...
    fun testPseudolocalizeBasicString() {
        ...
        Truth.assertThat(newString3.ref.value())
            .isEqualTo(
                "${bidiWordStart}H${bidiWordEnd}ell${bidiWordStart}o$bidiWordEnd " +
    ...
    fun testPseudolocalizeStyledString() {
        val pool = StringPool()
        val spans = listOf(Span("i", 1, 10), Span("b", 2, 3), Span("b", 6, 7))
    ...
    fun testPseudolocalizeNestedAndAdjacentTags() {
        val pool = StringPool()
        val originalSpans =
            listOf(Span("b", 16, 19), Span("em", 29, 47), Span("i", 38, 40), Span("b", 44, 47))
        val originalText = "This sentence is not what you think it is at all."
        val original =
            StyledString(pool.makeRef(StyleString(originalText, originalSpans)), listOf())
        ...
        Truth.assertThat(spans[2].firstChar)
            .isEqualTo("[Ţĥîš šéñţéñçé îš ñöţ ŵĥåţ ýöû ţĥîñķ îţ".length)
    ...
    fun testPluralsArePseudolocalized() {
        ...
        table.addResource(
            parseResourceName("com.pkg:plurals/foo")!!.resourceName,
            ConfigDescription(),
            "",
            plural,
        )
    ...
    fun canPseudolocalizeNestedTagWithSpace() {
        val pool = StringPool()
        val originalSpans = listOf(Span("b", 0, 4), Span("i", 0, 10))
        val originalText = "hello world"
        val original =
            StyledString(pool.makeRef(StyleString(originalText, originalSpans)), listOf())
