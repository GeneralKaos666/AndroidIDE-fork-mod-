...
class ResTableConfigTest {
    ...
    fun testBlockMethods() {
        ...
        // Blocks are Little Endian and therefore the lower bits come first in the block. In other
        // words
        // if the mcc is the "first" part of the imsi block it should be equal to the lower two
        // bytes of
    ...
    private fun matchTestUnequal(
        default: ResTableConfig,
        first: ResTableConfig,
        second: ResTableConfig,
    ) {
    ...
    private fun matchTestOneWay(
        default: ResTableConfig,
        matches: ResTableConfig,
        noMatch: ResTableConfig,
    ) {
        // Default should match everything.
    ...
    fun testMatches() {
            ...
            ResTableConfig(
                screenLayout =
                    (ResTableConfig.SCREEN_LAYOUT.SIZE_LARGE.toInt() or
                            ResTableConfig.SCREEN_LAYOUT.SCREENLONG_YES.toInt())
                        .toByte()
            )
        ...
        val uiMode =
            ResTableConfig(
                uiMode =
                    (ResTableConfig.UI_MODE.NIGHT_YES.toInt() or
                            ResTableConfig.UI_MODE.TYPE_APPLIANCE.toInt())
                        .toByte()
            )
        ...
        val colorMode =
            ResTableConfig(
                colorMode =
                    (ResTableConfig.COLOR_MODE.HDR_NO.toInt() or
                            ResTableConfig.COLOR_MODE.WIDE_GAMUT_YES.toInt())
                        .toByte()
            )
    ...
    fun testLocaleIsMoreSpecificThan() {
        val default = ResTableConfig()
        val languageOnly =
            ResTableConfig(language = byteArrayOf('e'.code.toByte(), 'n'.code.toByte()))
        val countryOnly =
            ResTableConfig(country = byteArrayOf('U'.code.toByte(), 'S'.code.toByte()))
        ...
        val localeScript = ResTableConfig(localeScript = byteArrayOf(0x01, 0x02, 0x03, 0x04))
        val numberingSystem =
            ResTableConfig(
        ...
        // localeScript being computed is not considered. Therefore numbering system is next on the
        // list
        ...
        for (i in firstToLast.indices) {
            for (j in (i + 1).until(firstToLast.size)) {
                ...
                try {
                    ...
                    throw AssertionError(
                        "Failed with less = \"$less\" at index $i and more = \"$more\" at index $j.",
                        e,
                    )
                }
            }
    ...
    fun testLocaleCompare() {
        ...
        val localeNoCountryWithScript1 =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                localeScript = byteArrayOf(1, 2, 3, 4),
            )
        val localeNoCountryWithScript2 =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                localeScript = byteArrayOf(1, 2, 3, 5),
            )
        val localeNoCountry2 =
            ResTableConfig(language = byteArrayOf('f'.code.toByte(), 'r'.code.toByte()))
        val localeNoCountryWithScript3 =
            ResTableConfig(
        ...
        val localeBasicWithScript =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                country = ResTableConfig.UNITED_STATES,
                localeScript = byteArrayOf(1, 2, 3, 4),
            )
        val localeBasicWithScriptAndVariant =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                country = ResTableConfig.UNITED_STATES,
                localeScript = byteArrayOf(1, 2, 3, 4),
        ...
        val localeBasicWithScriptAndVariant2 =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                country = ResTableConfig.UNITED_STATES,
                localeScript = byteArrayOf(1, 2, 3, 4),
        ...
        val localeBasicWithScriptVariantAndNumberSystem =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                country = ResTableConfig.UNITED_STATES,
                localeScript = byteArrayOf(1, 2, 3, 4),
        ...
        val localeBasicWithScriptVariantAndNumberSystem2 =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                country = ResTableConfig.UNITED_STATES,
                localeScript = byteArrayOf(1, 2, 3, 4),
        ...
        val localeBasicWithComputedScript =
            ResTableConfig(
                language = ResTableConfig.ENGLISH,
                country = ResTableConfig.UNITED_STATES,
                localeScriptWasComputed = true,
    ...
    fun testDiff() {
        ...
        val second =
            ResTableConfig(mcc = 310, language = ResTableConfig.ENGLISH, screenHeight = 480)
        val third =
            ResTableConfig(
                sdkVersion = 23,
                screenWidth = 480,
    ...
    private fun isBetterLocaleTransitive(
        ...
        vararg leastToGreatest: ResTableConfig,
    ) {
        ...
        for (i in leastToGreatest.indices) {
            for (j in (i + 1).until(leastToGreatest.size)) {
                ...
                try {
                    ...
                    throw AssertionError(
                        "Failed with worse = \"$worse\" at index $i and better = \"$better\" at index $j.",
                        e,
                    )
                }
            }
    ...
    fun testIsBetterLocale() {
        ...
        Truth.assertThat(localeVariantFail.isLocaleBetterThan(localeNumberSystem, requested))
            .isFalse()
        Truth.assertThat(localeNumberSystem.isLocaleBetterThan(localeVariantFail, requested))
            .isTrue()
        Truth.assertThat(localeVariantFail.isLocaleBetterThan(languageAndCountry, requested))
            .isFalse()
    }
    ...
    fun testIsBetterThan() {
        ...
        val screenSize2 =
            ResTableConfig(
                screenWidth = requested.screenWidth,
                screenHeight = requested.screenHeight,
            )
        ...
        val navHidden =
            ResTableConfig(
                inputFlags =
                    (requested.inputFlags.toInt() and ResTableConfig.INPUT_FLAGS.NAVHIDDEN_MASK)
                        .toByte()
            )
        val keysHiddenAlmostMatch =
            ResTableConfig(inputFlags = ResTableConfig.INPUT_FLAGS.KEYSHIDDEN_NO)
        val keysHiddenMatch =
            ResTableConfig(
                inputFlags =
                    (requested.inputFlags.toInt() and ResTableConfig.INPUT_FLAGS.KEYSHIDDEN_MASK)
                        .toByte()
            )
        ...
        val nightMode =
            ResTableConfig(
                uiMode = (requested.uiMode.toInt() and ResTableConfig.UI_MODE.NIGHT_MASK).toByte()
            )
        val uiType =
            ResTableConfig(
                uiMode = (requested.uiMode.toInt() and ResTableConfig.UI_MODE.TYPE_MASK).toByte()
            )
        val orientation = ResTableConfig(orientation = requested.orientation)
        val hdr =
            ResTableConfig(
                colorMode =
                    (requested.colorMode.toInt() and ResTableConfig.COLOR_MODE.HDR_MASK).toByte()
            )
        val gamut =
            ResTableConfig(
                colorMode =
                    (requested.colorMode.toInt() and ResTableConfig.COLOR_MODE.WIDE_GAMUT_MASK)
                        .toByte()
            )
        val round =
            ResTableConfig(
                screenLayout2 =
                    (requested.screenLayout2.toInt() and
                            ResTableConfig.SCREEN_LAYOUT2.SCREENROUND_MASK)
                        .toByte()
            )
        val long =
            ResTableConfig(
                screenLayout =
                    (requested.screenLayout.toInt() and
                            ResTableConfig.SCREEN_LAYOUT.SCREENLONG_MASK)
                        .toByte()
            )
        val sizeFlag =
            ResTableConfig(
                screenLayout =
                    (requested.screenLayout.toInt() and ResTableConfig.SCREEN_LAYOUT.SIZE_MASK)
                        .toByte()
            )
        val screenSizeDp1 = ResTableConfig(screenWidthDp = requested.screenWidthDp)
        val screenSizeDp2 =
            ResTableConfig(
                screenWidthDp = requested.screenWidthDp,
                screenHeightDp = requested.screenHeightDp,
            )
        val smallestScreenWidth1 =
            ResTableConfig(smallestScreenWidthDp = requested.smallestScreenWidthDp - 20)
        // Closer to requested will match better.
        val smallestScreenWidth2 =
            ResTableConfig(smallestScreenWidthDp = requested.smallestScreenWidthDp)
        val screenDir =
            ResTableConfig(
                screenLayout =
                    (requested.screenLayout.toInt() and ResTableConfig.SCREEN_LAYOUT.DIR_MASK)
                        .toByte()
            )
    ...
    fun testToString() {
        ...
        val config9 =
            ResTableConfig(
                ...
                (ResTableConfig.INPUT_FLAGS.NAVHIDDEN_YES.toInt() or
                        ResTableConfig.INPUT_FLAGS.KEYSHIDDEN_SOFT.toInt())
                    .toByte(),
                ...
                (ResTableConfig.UI_MODE.NIGHT_NO.toInt() or
                        ResTableConfig.UI_MODE.TYPE_DESK.toInt())
                    .toByte(),
                480,
                480,
                640,
        ...
        Truth.assertThat(config9.toString())
            .isEqualTo(
