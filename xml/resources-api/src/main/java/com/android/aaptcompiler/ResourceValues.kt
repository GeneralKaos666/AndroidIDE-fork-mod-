        ...
        val dataType =
            when {
                referenceType == RESOURCE ->
                    if (dynamic) ResValue.DataType.DYNAMIC_REFERENCE
                    else ResValue.DataType.REFERENCE
                else ->
                    if (dynamic) ResValue.DataType.DYNAMIC_ATTRIBUTE
                    else ResValue.DataType.ATTRIBUTE
            }
...
data class AttributeResource(var typeMask: Int = 0, val logger: ILogger? = null) : Value() {
    ...
    fun matches(item: Item): Boolean {
        ...
        // Enums and flags are encoded as integers, so check them first before doing any range
        // checks.
    ...
    fun isCompatibleWith(other: AttributeResource): Boolean {
        // if the high bits are set on any of these attribute type masks, then they are
        // incompatible.
...
private fun maskToString(typeMask: Int): String {
    ...
    result.append(
        maskToString(typeMask, Resources.Attribute.FormatFlags.BOOLEAN_VALUE, "| boolean")
    )
