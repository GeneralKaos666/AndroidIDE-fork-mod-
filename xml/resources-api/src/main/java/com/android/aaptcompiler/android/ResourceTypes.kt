...
data class ResValue(val dataType: DataType, val data: Int, val size: Short = 0) {
    ...
    enum class DataType(val byteValue: Byte) {
        // The 'data' is either 0 or 1, specifying this resource is undefined or empty,
        // respectively.
        NULL(0X00),
        ...
        // The 'data' holds a dynamic ResTable reference, which needs to be resolved before it can
        // be
        // used as a REFERENCE.
        DYNAMIC_REFERENCE(0x07),

        // The 'data' holds an attribute resource identifier, which needs to be resolved before it
        // can
    ...
    companion object {
        fun get(buffer: ByteBuffer, length: Int): ResStringPool {
            ...
            if (header.styleCount != 0) {
                ...
                for (i in 0.until(header.styleCount)) {
                    val styleLocation =
                        buffer.getInt(currentStyleIndex).deviceToHost() + header.stylesStart
                    currentStyleIndex += 4
        ...
        private fun decodeString(buffer: ByteBuffer, location: Int, utf8: Boolean): String {
            ...
            if (utf8) {
                ...
                val utf16Length =
                    when {
                        (firstByteUTF16.toInt() and StringPool.TWO_BYTE_UTF8_LENGTH_SIGNIFIER) !=
                            0 -> {
                            val secondByte = buffer.get(stringPosition).toInt() and 0xff
                            ++stringPosition

                            ((firstByteUTF16.toInt() shl 8) + secondByte) and
                                StringPool.UTF8_ENCODE_LENGTH_MAX
                        }
                ...
                val utf8Length =
                    when {
                        (firstByteUTF8.toInt() and StringPool.TWO_BYTE_UTF8_LENGTH_SIGNIFIER) !=
                            0 -> {
                            val secondByte = buffer.get(stringPosition).toInt() and 0xff
                            ++stringPosition

                            ((firstByteUTF8.toInt() shl 8) + secondByte) and
                                StringPool.UTF8_ENCODE_LENGTH_MAX
                        }
                ...
                if (utf16String.length != utf16Length) {
                    error(
                        "Invalid StringPool: specified UTF16 does not match actual string length."
                    )
                }
                ...
                val utf16Length =
                    when {
                        (firstShort.toInt() and StringPool.TWO_CHAR_UTF16_LENGTH_SIGNIFIER) !=
                            0 -> {
                            val secondShort =
                                buffer.getShort(stringPosition).deviceToHost().toInt() and 0xffff
                            stringPosition += 2
                            ((firstShort.toInt() shl 16) + secondShort) and
                                StringPool.UTF16_ENCODE_LENGTH_MAX
                        }
...
fun stringToFloat(string: String): ResValue? {
    ...
    if (parsedValue == null) {
        // Kotlin does not parse hex floating point numbers that do not have exponent identifier.
        // I.e.
