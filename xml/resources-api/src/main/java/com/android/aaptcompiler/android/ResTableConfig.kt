...
open class ResTableConfig(
    ...
    fun getScreenConfig2() =
        ((screenLayout2.toInt() and 0xff) or
    ...
    fun diff(other: ResTableConfig): Int {
        ...
        if (
            inputFlags.toInt() and (INPUT_FLAGS.KEYSHIDDEN_MASK or INPUT_FLAGS.NAVHIDDEN_MASK) !=
                other.inputFlags.toInt() and
                    (INPUT_FLAGS.KEYSHIDDEN_MASK or INPUT_FLAGS.NAVHIDDEN_MASK)
        ) {
    ...
    fun compareLocales(other: ResTableConfig): Int {
        ...
        // The language & region are equal, so compare the scripts, variants and numbering systems
        // in
        // that order. Comparison of variants and numbering systems should happen very infrequently
        // (if
        // at all.)
    ...
    fun isMoreSpecificThan(other: ResTableConfig): Boolean {
        ...
        if (uiMode.isTruthy() || other.uiMode.isTruthy()) {
            if (
                uiMode.toInt() and UI_MODE.TYPE_MASK != other.uiMode.toInt() and UI_MODE.TYPE_MASK
            ) {
                return (uiMode.toInt() and UI_MODE.TYPE_MASK).isTruthy()
            }
            if (
                uiMode.toInt() and UI_MODE.NIGHT_MASK != other.uiMode.toInt() and UI_MODE.NIGHT_MASK
            ) {
     ...
     * match the requested configuration at all; if they are not first filtered, non-matching
     * results can be considered better than matching ones.
     *
     * <p> The general rule per attribute: if the request cares about an attribute (it normally
     * does), if the two ('this' and 'other') are equal it's a tie. If they are not equal then one
     * must be generic because only generic and '==requested' will pass the match() call. So if this
     * is not generic, it wins. If this IS generic, 'other' wins (return false).
     */
    fun isBetterThan(other: ResTableConfig, requested: ResTableConfig?): Boolean {
        ...
        if (smallestScreenWidthDp.isTruthy() || other.smallestScreenWidthDp.isTruthy()) {
            // The configuration closest to the actual size is best. We assume that larger configs
            // have
        ...
        if (getScreenSizeDp().isTruthy() || other.getScreenSizeDp().isTruthy()) {
            ...
            // already been filtered.  Note that if a particular dimension is unspecified, we will
            // end up
        ...
        if (screenLayout.isTruthy() || other.screenLayout.isTruthy()) {
            ...
            if (
                ...
                if (
                    (requested.screenLayout.toInt() and SCREEN_LAYOUT.SIZE_MASK) >
                ...
                // For screen size, the best match is the one that is closest to the requested
                // screen size,
                // but not over (the not over part is dealt with in match() below).
                return if (fixedMySL == fixedOtherSL) {
                    // If the two are the same, but 'other' is actually undefined, then 'this' is
                    // really a
                    // better match.
                    mySL != 0
                } else {
        ...
        if (uiMode.isTruthy() || other.uiMode.isTruthy()) {
            ...
            if (
                myType != otherType && (requested.uiMode.toInt() and UI_MODE.TYPE_MASK).isTruthy()
            ) {
            ...
            if (
                myNight != otherNight &&
                    (requested.uiMode.toInt() and UI_MODE.NIGHT_MASK).isTruthy()
            ) {
        ...
        if (getScreenType().isTruthy() || other.getScreenType().isTruthy()) {
            if (density != other.density) {
                ...
                // DENSITY_ANY is now dealt with. We should look to pick a density bucket and
                // potentially
                // scale it. Any density is potentially useful because the system will scale it.
                // Scaling
        ...
        if (getInput().isTruthy() || other.getInput().isTruthy()) {
            ...
            if (keysHidden != otherKeysHidden) {
                val requestedKeysHidden =
                    requested.inputFlags.toInt() and INPUT_FLAGS.KEYSHIDDEN_MASK
                if (requestedKeysHidden.isTruthy()) {
                    when {
                        !keysHidden.isTruthy() -> return false
                        !otherKeysHidden.isTruthy() -> return true
                        // For compatibility, we count KEYSHIDDEN_NO as being the same as
                        // KEYSHIDDEN_SOFT.  Here
    ...
    fun isLocaleBetterThan(other: ResTableConfig, requested: ResTableConfig): Boolean {
        ...
        // Non-matching locales have been filtered out, so both resources match the requested
        // locale.
        //
        ...
        if (!langsAreEquivalent(language, other.language)) {
            // The languages of the two resources are not equivalent. If we are here, we can only
            // assume
            // that the two resources matched the request because one doesn't have a language and
            // the
            // other has a matching language.
            //
            // We consider the one that has the language specified a better match.
            //
            // The exception is that we consider no-language resources a better match for US English
            // and
            // similar locales than locales that are a descendant of International English (en-001),
            // since
            // no-language resources are where the US English resource have traditionally lived for
            // most
            // apps.
            if (requested.language contentEquals ENGLISH) {
                when {
                    requested.country contentEquals UNITED_STATES -> {
                        // For US English itself, we consider a no-locale resource a better match
                        // than if the
                        ...
                        return if (language[0].isTruthy()) {
                            ...
                            !(other.country[0] == 0.toByte() ||
                                other.country contentEquals UNITED_STATES)
                        }
                    }
        ...
        //
        // Because the languages are equivalent, computeScript() always returns a non-empty script
        // for
        // languages it knows about, and we have passed the script checks in match(), the scripts
        // are
        // either all unknown or are all the same. So we can't gain anything by checking the
        // scripts. We
        ...
        val otherNumberingMatches =
            other.localeNumberSystem contentEquals requested.localeNumberSystem
    ...
    fun match(settings: ResTableConfig): Boolean {
        ...
        if (getLocale().isTruthy()) {
            // Don't consider country and variants when deciding matches. (Theoretically, the
            // variant can
            // also affect the script. For example, "ar-alalc97" probably implies the Latin script,
            // but
            // since CLDR doesn't support getting likely scripts for that, we'll assume the variant
            // doesn't change the script.)
            //
            // If two configs differ only in their country and variant, they can be weeded out in
            // the
            ...
            // For backward compatibility and supporting private-use locales, we fall back to old
            // behavior
            // if we couldn't determine the script for either of the desired locale or the provided
            // locale. But if we could determine the scripts, they should be the same for the
            // locales to
            // match.
        ...
        if (
            grammaticalInflection.isTruthy() &&
                grammaticalInflection != settings.grammaticalInflection
        ) {
            return false
        }
        ...
        if (getScreenConfig().isTruthy()) {
            ...
            if (
                smallestScreenWidthDp.isTruthy() &&
        ...
        if (getInput().isTruthy()) {
            ...
            if (keysHidden.isTruthy() && keysHidden != setKeysHidden) {
                // For compatibility, we count a request for KEYSHIDDEN_NO as also matching the more
                // recent
                // KEYSHIDDEN_SOFT.  Basically KEYSHIDDEN_NO means there is some kind of keyboard
                // available.
                if (
    ...
    companion object {
        ...
        val UNITED_STATES =
            byteArrayOf('U'.code.toByte(), 'S'.code.toByte()) // packed version of "US"
        ...
        internal fun unpackLanguageOrRegion(input: ByteArray, base: Byte) =
            when {
                (input[0].toInt() and 0x80).isTruthy() -> {
                    ...
                    val secondChar =
                        (((input[1].toInt() and 0xe0) shr 5) +
                                ((input[0].toInt() and 0x03) shl 3) +
                                base)
                            .toByte()
        ...
        internal fun createConfig(buffer: ByteBuffer): ResTableConfig {
            ...
            val screenConfig2 =
                if (sizeOnHost >= SCREEN_CONFIG_EXTENSION_MIN_SIZE) buffer.int else 0
        ...
        internal fun screenWidthFromScreenSize(screenSize: Int) =
            screenSize.deviceToHost() and 0xffff
        ...
        internal fun minorVersionFromVersion(version: Int) =
            (version.deviceToHost() ushr 16).toShort()
    ...
    object SCREEN_LAYOUT {
        ...
        const val DIR_ANY =
            (AConfiguration.ACONFIGURATION_LAYOUTDIR_ANY.toInt() shl DIR_SHIFT).toByte()
        const val DIR_LTR =
            (AConfiguration.ACONFIGURATION_LAYOUTDIR_LTR.toInt() shl DIR_SHIFT).toByte()
        const val DIR_RTL =
            (AConfiguration.ACONFIGURATION_LAYOUTDIR_RTL.toInt() shl DIR_SHIFT).toByte()
