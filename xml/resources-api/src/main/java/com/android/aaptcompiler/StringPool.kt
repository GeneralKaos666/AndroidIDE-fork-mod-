...
class StringPool {
    ...
    private fun flatten(out: BigBuffer, utf8: Boolean, logger: ILogger?) {
        ...
        if (stringIndexBlock != null) {
            // Styles come first
            for (entry in styles) {
                stringIndexBlock.writeInt(
                    (out.size - beginStringsIndex).hostToDevice(),
                    currentStringIndex,
                )
        ...
        if (stylesIndexBlock != null) {
            ...
            for (entry in styles) {
                stylesIndexBlock.writeInt(
                    (out.size - header.stylesStart).hostToDevice(),
                    currentStyleIndex,
                )
                currentStyleIndex += 4
    ...
    private fun encodeSpan(
        resSpan: ResStringPoolSpan,
        out: BigBuffer.BlockRef,
        location: Int,
    ): Int {
    ...
    private fun encodeString(str: String, utf8: Boolean, out: BigBuffer) {
        if (utf8) {
            ...
            // Make sure the lengths to be encoded do not exceed the maximum length that can be
            // encoded
            // using chars
            ...
            // Make sure the length to be encoded does not exceed the maximum possible length that
            // can be
            // encoded
    ...
    private fun sortStyles(comparator: Comparator<Context>?) {
        val entryComparator =
            if (comparator != null) {
                compareBy<StyleEntry, Context>(comparator) { it.context }
                    .thenComparing(STYLE_ON_VALUE)
            } else {
                STYLE_ON_VALUE
            }
