...
class CompletionListAdapter : EditorCompletionAdapter() {
    ...
    private fun setItemBackground(binding: LayoutCompletionItemBinding, isCurrent: Boolean) {
        val color =
            if (isCurrent) getThemeColor(SchemeAndroidIDE.COMPLETION_WND_BG_CURRENT_ITEM) else 0
                ...
                when (data) {
                    ...
                    is MemberCompletionData -> {
                        if (data is MethodCompletionData) {
                            ...
                            // build the method identifier by joining the method name and the erased
                            // parameter types
                            // for method 'int some(String)', the identifier becomes
