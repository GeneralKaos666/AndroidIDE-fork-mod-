...
class SchemeParser(private val resolveFileRef: (String) -> File) {
    ...
    enum class EditorColors(val key: String, val id: Int) {
        ...
        COMPLETION_WINDOW_OUTLINE(
            "completion.window.outline",
        ...
        COMPLETION_WND_TEXT_API(
            "completion.window.text.api",
            SchemeAndroidIDE.COMPLETION_WND_TEXT_API,
        ),
            ...
            fun forKey(key: String): EditorColors {
                return values().find { it.key == key }
                    ?: throw IllegalArgumentException(
                        "No editor color scheme available for key: '$key'"
                    )
            }
        }
    }
        ...
        JsonReader(scheme.file.reader()).use { reader ->
            ...
            if (scheme.name.isBlank()) {
                throw ParseException(
                    "A color scheme must a valid name. Current name is '${scheme.name}'"
                )
            }
        }
    }
