        ...
        for (token in tokens.tokens) {
            ...
            when (type) {
                GroovyLexer.WS -> {
                    if (first) {
                        spans.add(
                            SpanFactory.obtain(
                                offset,
                                TextStyle.makeStyle(SchemeAndroidIDE.TEXT_NORMAL),
                            )
                        )
                        first = false
                    }
                ...
                GroovyLexer.NULL_LITERAL ->
                    spans.add(
                        SpanFactory.obtain(offset, TextStyle.makeStyle(SchemeAndroidIDE.LITERAL))
                    )
                ...
                GroovyLexer.DOT ->
                    spans.add(
                        SpanFactory.obtain(offset, TextStyle.makeStyle(SchemeAndroidIDE.OPERATOR))
                    )
                ...
                GroovyLexer.SHORT ->
                    spans.add(
                        SpanFactory.obtain(offset, TextStyle.makeStyle(SchemeAndroidIDE.TYPE_NAME))
                    )

                GroovyLexer.COMMENT ->
                    spans.add(SpanFactory.obtain(offset, SchemeAndroidIDE.forComment()))
                GroovyLexer.LINE_COMMENT -> handleLineCommentSpan(token, spans, offset)
                GroovyLexer.AT ->
                    spans.add(
                        SpanFactory.obtain(offset, TextStyle.makeStyle(SchemeAndroidIDE.ANNOTATION))
                    )
