...
class TreeSitterIndentProvider(
    ...
    private fun computeIndentForLine(
        ...
        if (isEmptyLine) {
            ...
            if (node.type == "comment") {
                // The final node we capture of the previous line can be a comment node, which
                // should also be ignored
                // Unless the last line is an entire line of comment, ignore the comment range and
                // find the last node again
        ...
        // this is to ensure that we do not accidentally apply multiple indent levels to the same
        // line
        ...
        while (node != null && node.canAccess()) {
            ...
            // do not indent for nodes that starts-and-ends on same line and starts on target line
            // (lnum)
            ...
            if (
                shouldProcess &&
                    (indents.hasNode(IDENT_BEGIN, node) &&
                        ...
                        (srow != line ||
                            indents.hasMeta(IDENT_BEGIN, node, "indent.start_at_same_line")))
            ) {
                indent += indentSize
                isProcessed = true
            }
                ...
                if (oDelimNode != null) {
                    ...
                    if (oIsLastInLine) {
                        ...
                        if (shouldProcess) {
                            indent += indentSize
                            if (cIsLastInLine) {
                                // If current line is outside the range of a node marked with
                                // `@aligned_indent`
                                // Then its indent level shouldn't be affected by `@aligned_indent`
                                // node
                    ...
                    if (csrow != null && csrow != osrow && csrow == line) {
                        ...
                        avoidLastMatchingNext =
                            meta.getBolean("indent.avoid_last_matching_next") ?: false
                    }
                    ...
                    if (avoidLastMatchingNext) {
                        ...
                        // hanging indents)
                        val osrowIndent =
                            TextUtils.countLeadingSpaceCount(content.getLine(osrow), indentSize)
                    ...
                    isProcessed = true
                    if (indentIsAbsolute) {
                        // don't allow further indenting by parent nodes, this is an absolute
                        // position
                        return indent
                    }
                }
    ...
    private inner class IndentsContainer {
        ...
        fun hasMeta(type: String, node: TSNode, metaKey: String) =
