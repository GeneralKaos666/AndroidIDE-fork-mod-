...
class PartialReparserImpl : PartialReparser {
    ...
    override fun reparseMethod(
        ...
        if (origStartPos > origEndPos) {
            log.warn(
                "Java returned start position: {} > end position: {}",
                origStartPos,
                origEndPos,
            )
            return false
    ...
    private fun newParser(
        ...
        if (
            endPositions
                is
                com.itsaky.androidide.javac.services.NBParserFactory.NBJavacParser.EndPosTableImpl
        ) {
            ...
            if (body.stats.isEmpty() || !TreeInfo.isSelfCall(body.stats.head)) {
                body.stats =
                    body.stats.prepend(
                        make
                            .at(body.pos)
                            .Exec(make.Apply(List.nil(), make.Ident(names._super), List.nil()))
                    )
            } else if (
