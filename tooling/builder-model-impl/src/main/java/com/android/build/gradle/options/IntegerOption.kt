...
enum class IntegerOption(override val propertyName: String, stage: ApiStage) : Option<Int> {
    ANDROID_TEST_SHARD_COUNT(
        "android.androidTest.numShards",
        ApiStage.Removed(VERSION_8_2, "Cross device sharding is no longer supported."),
    ),
     ...
     *
     * The model-only mode is triggered when the IDE does a sync, and therefore we do things a bit
     * differently (don't throw exceptions for instance). Things evolved a bit over versions and the
     * behavior changes. This reflects the mode to use.
     *
    ...
    /**
     * Flags for Android Test Retention >=1: enable Android Test Retention and set it as maximum
     * number of snapshots. 0: disable Android Test Retention. <0: enable Android Test Retention
     * with unlimited number of snapshots.
     */
    TEST_FAILURE_RETENTION(
    ...
    MANAGED_DEVICE_SHARD_POOL_SIZE(
        "android.experimental.androidTest.numManagedDeviceShards",
        ApiStage.Experimental,
    ),

    /**
     * The timeout duration in minute for Gradle Managed Device setup steps (booting AVD and
     * creating snapshot image) If the value is 0 or negative value, it will never time out.
     */
    GRADLE_MANAGED_DEVICE_SETUP_TIMEOUT_MINUTES(
        "android.experimental.testOptions.managedDevices.setupTimeoutMinutes",
        ApiStage.Experimental,
    ),

    /**
     * Maximum number of concurrent Gradle Managed Devices (AVDs) to be active at any one point in
     * time.
     *
     * If the value is 0 or negative value, there is no maximum devices.
     */
    GRADLE_MANAGED_DEVICE_MAX_CONCURRENT_DEVICES(
        "android.experimental.testOptions.managedDevices.maxConcurrentDevices",
        ApiStage.Experimental,
    ),

    /**
     * The timeout duration in seconds for install apk If the value is 0 or negative value, it will
     * be set to a default value by UTP.
     */
    INSTALL_APK_TIMEOUT(
    ...
    THREAD_POOL_SIZE(
        ...
        ApiStage.Removed(VERSION_BEFORE_4_0, "The android.threadPoolSize property has no effect"),
    );
