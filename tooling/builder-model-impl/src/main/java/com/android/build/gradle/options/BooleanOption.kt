...
enum class BooleanOption(
    ...
    // IDE_GENERATE_SOURCES_ONLY(AndroidProject.PROPERTY_GENERATE_SOURCES_ONLY, false,
    // ApiStage.Stable),
    ...
    /**
     * Setting this field to false indicates that in the current project, all the APKs installed
     * during test will be uninstalled after test finishes. Setting it to true means that the AGP
     * will leave the test APKs untouched after test.
     *
     * Default is false
     */
    ANDROID_TEST_LEAVE_APKS_INSTALLED_AFTER_RUN(
     ...
     * ([com.android.build.gradle.internal.publishing.AndroidArtifacts.ArtifactScope.EXTERNAL]) in
     * addition to the input artifact's dependencies provided by Gradle through
    ...
    INCLUDE_DEPENDENCY_INFO_IN_APKS(
        "android.includeDependencyInfoInApks",
        true,
        FeatureStage.Supported,
    ),
    ...
    BUILD_FEATURE_MLMODELBINDING(
        "android.defaults.buildfeatures.mlmodelbinding",
        false,
    ...
    DISALLOW_DEPENDENCY_RESOLUTION_AT_CONFIGURATION(
        "android.dependencyResolutionAtConfigurationTime.disallow",
        false,
    ...
    CONSUME_DEPENDENCIES_AS_SHARED_LIBRARIES(
        "android.consumeDependenciesAsSharedLibraries",
        false,
        FeatureStage.Experimental,
    ),
    DISABLE_EARLY_MANIFEST_PARSING(
        "android.disableEarlyManifestParsing",
        false,
        FeatureStage.Experimental,
    ),
    ...
    KEEP_SERVICES_BETWEEN_BUILDS(
        "android.keepWorkerActionServicesBetweenBuilds",
        false,
        FeatureStage.Experimental,
    ),
    ENABLE_PARTIAL_R_INCREMENTAL_BUILDS(
    ...
    DISABLE_MINSDKLIBRARY_CHECK(
        "android.unsafe.disable.minSdkLibraryCheck",
        false,
        FeatureStage.Experimental,
    ),
    ...
    /**
     * When enabled, incompatible APKs installed on a testing device will be uninstalled
     * automatically during an instrumentation test run (e.g. When
     * INSTALL_FAILED_UPDATE_INCOMPATIBLE error happens after attempting to install APKs for
     * testing).
     */
    UNINSTALL_INCOMPATIBLE_APKS(
        "android.experimental.testOptions.uninstallIncompatibleApks",
        false,
        FeatureStage.Experimental,
    ),
     ...
     * its instability.
     */
    GRADLE_MANAGED_DEVICE_ALLOW_OLD_API_LEVEL_DEVICES(
    ...
    /**
     * When set R classes are treated as compilation classpath in libraries, rather than runtime
     * classpath, with values set to 0.
     */
    ...
    ENABLE_PARALLEL_NATIVE_JSON_GEN(
    ...
    ENABLE_NATIVE_COMPILER_SETTINGS_CACHE(
        "android.enableNativeCompilerSettingsCache",
        false,
        FeatureStage.Experimental,
    ),
    ENABLE_CMAKE_BUILD_COHABITATION(
        "android.enableCmakeBuildCohabitation",
        false,
        FeatureStage.Experimental,
    ),
    ENABLE_PROGUARD_RULES_EXTRACTION(
        "android.proguard.enableRulesExtraction",
        true,
        FeatureStage.Experimental,
    ),
    USE_DEPENDENCY_CONSTRAINTS(
    ...
    ENABLE_DUPLICATE_CLASSES_CHECK(
        "android.enableDuplicateClassesCheck",
        true,
        FeatureStage.Experimental,
    ),
    MINIMAL_KEEP_RULES("android.useMinimalKeepRules", true, FeatureStage.Experimental),
    EXCLUDE_RES_SOURCES_FOR_RELEASE_BUNDLES(
        "android.bundle.excludeResSourcesForRelease",
        true,
        FeatureStage.Experimental,
    ),
    ENABLE_BUILD_CONFIG_AS_BYTECODE(
    ...
    FORCE_DETERMINISTIC_APK(
        "android.experimental.forceDeterministicApk",
        false,
        FeatureStage.Experimental,
    ),
    ...
    VERIFY_AAR_CLASSES(
        "android.experimental.verifyLibraryClasses",
        false,
        FeatureStage.Experimental,
    ),
    DISABLE_COMPILE_SDK_CHECKS(
        "android.experimental.disableCompileSdkChecks",
        false,
        FeatureStage.Experimental,
    ),
    ADDITIONAL_ARTIFACTS_IN_MODEL(
        "android.experimental.additionalArtifactsInModel",
        false,
        FeatureStage.Experimental,
    ),
    ...
    SUPPORT_PAST_STUDIO_VERSIONS(
        "android.experimental.support.past.studio.versions",
        false,
        FeatureStage.Experimental,
    ),
    ...
    LINT_USE_K2_UAST("android.lint.useK2Uast", false, FeatureStage.Experimental),
     ...
     */
    @Suppress("unused")
    PREFER_CMAKE_FILE_API("android.preferCmakeFileApi", true, FeatureStage.Enforced(VERSION_7_0)),
    @Suppress("unused")
    ...
    ENABLE_NEW_RESOURCE_PROCESSING(
        ...
        true,
        FeatureStage.Enforced(VERSION_BEFORE_4_0, "New resource processing is now always enabled."),
    ),
    @Suppress("unused")
    ...
    ENABLE_GRADLE_WORKERS(
        ...
        true,
        FeatureStage.Enforced(VERSION_4_2, "Gradle workers are always used."),
    ),
    @Suppress("unused")
    ENABLE_D8(
        "android.enableD8",
    ...
    ENABLE_D8_DESUGARING(
        ...
        true,
        FeatureStage.Enforced(VERSION_7_0, "D8 desugaring is used by default, when applicable."),
    ),
    @Suppress("unused")
    ENABLE_R8_DESUGARING(
        "android.enableR8.desugaring",
        true,
        FeatureStage.Enforced(VERSION_7_0, "R8 desugaring is used by default, when applicable."),
    ),
    USE_NEW_LINT_MODEL(
        "android.experimental.useNewLintModel",
        true,
        FeatureStage.Enforced(VERSION_7_0),
    ),

    /** Whether Jetifier will skip libraries that already support AndroidX. */
    JETIFIER_SKIP_IF_POSSIBLE(
        "android.jetifier.skipIfPossible",
        true,
        FeatureStage.Enforced(VERSION_7_0),
    ),
    @Suppress("unused")
    ...
    ENABLE_INCREMENTAL_DEXING_TASK_V2(
        "android.enableIncrementalDexingTaskV2",
        true,
        FeatureStage.Enforced(VERSION_7_0),
    ),
    ...
    ENABLE_INCREMENTAL_DEXING_TRANSFORM(
        "android.enableIncrementalDexingTransform",
        true,
        FeatureStage.Enforced(VERSION_7_0),
    ),
    ENABLE_R8_LIBRARIES("android.enableR8.libraries", true, FeatureStage.Enforced(VERSION_7_0)),
    ENABLE_SYMBOL_TABLE_CACHING(
        "android.enableSymbolTableCaching",
        true,
        FeatureStage.Enforced(VERSION_7_0),
    ),
    ENABLE_JVM_RESOURCE_COMPILER(
        "android.enableJvmResourceCompiler",
        true,
        FeatureStage.Enforced(VERSION_7_0),
    ),

    /** Whether to use lint's partial analysis functionality. */
    USE_LINT_PARTIAL_ANALYSIS(
        "android.enableParallelLint",
        true,
        FeatureStage.Enforced(VERSION_7_2),
    ),
    ...
    ENABLE_TEST_SHARDING(
        ...
        false,
        FeatureStage.Removed(Version.VERSION_8_2, "Cross device sharding is no longer supported."),
    ); // end of enums
