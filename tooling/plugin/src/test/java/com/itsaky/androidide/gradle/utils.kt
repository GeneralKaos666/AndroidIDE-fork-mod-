...
internal fun writeInitScript(file: File, deps: List<File>) {
    ...
    val depsString =
        deps
            .filter { it.absolutePath.startsWith(root) }
            .joinToString(separator = System.lineSeparator()) { "classpath files(\"${it}\")" }
...
internal fun openProject(
    ...
    run {
        ...
        projectRoot
            .resolve("app/build.gradle" + if (useApplyPluginGroovySyntax) "" else ".kts")
            .toFile()
