        ...
        target.projectsLoaded { gradle ->
            gradle.rootProject.subprojects { sub ->
                if (!sub.buildFile.exists()) {
                    ...
                    // ':nested' represented as a 'Project', but it may or may not have a
                    // buildscript file
                    // if the project doesn't have a buildscript, then the plugins should not be
                    // applied
                    return@subprojects
                }
                ...
                sub.afterEvaluate {
                    logger.info(
                        "Trying to apply plugin '${BuildInfo.PACKAGE_NAME}' to project '${sub.path}'"
                    )
                ...
                if (mavenLocalRepo.isBlank()) {
                    ...
                    if (!repo.exists() || !repo.isDirectory) {
                        throw FileNotFoundException(
                            "Maven local repository '$mavenLocalRepo' not found"
                        )
                    }
