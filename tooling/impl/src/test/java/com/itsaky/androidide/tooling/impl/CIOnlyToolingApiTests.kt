...
class CIOnlyToolingApiTests {
    ...
    @JvmField @Rule val ciOnlyTestRule = CIOnlyTestRule()
    ...
    fun `test CI-only latest tested AGP version warning`() {
        ...
        val client =
            ToolingApiTestLauncher.MultiVersionTestClient(
        ...
        ToolingApiTestLauncher.launchServer(
            ...
            sysProps =
                mapOf(
                    ToolingProps.TESTING_LATEST_AGP_VERSION to
                        ToolingApiTestLauncher.MultiVersionTestClient.DEFAULT_AGP_VERSION
                ),
        ) {
            assertThat(result?.isSuccessful).isTrue()
