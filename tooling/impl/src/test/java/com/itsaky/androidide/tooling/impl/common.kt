...
internal fun performBasicProjectAssertions(project: IProject, server: IToolingApiServer) {
    ...
    val nested =
        javaDependencies.filterIsInstance(JavaModuleProjectDependency::class.java).filter {
            it.moduleName.endsWith("nested-java-library")
        }
    assertThat(nested).hasSize(2)
    assertThat(nested[0].projectPath).isNotEqualTo(nested[1].projectPath)
    assertThat(project.selectProject(StringParameter(":does-not-exist")).get().isSuccessful)
        .isFalse()
