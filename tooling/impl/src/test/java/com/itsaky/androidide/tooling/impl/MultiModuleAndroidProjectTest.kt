...
class MultiModuleAndroidProjectTest {
    ...
    fun `test project initialization with file path`() {
        val path =
            FileProvider.projectRoot()
                ...
                .resolve("should-be-directory.txt")
        path.deleteIfExists()
        path.createFile()
    ...
    fun `test task name should be shown if gradle configuration cache is enabled`() {
        ...
        val client =
            object :
                MultiVersionTestClient(excludeUnresolvedDependency = true, gradleVersion = "8.5") {

                override fun getBuildArguments(): CompletableFuture<List<String>> {
                    return CompletableFuture.completedFuture(
                        super.getBuildArguments().get().toMutableList().also {
        ...
        ToolingApiTestLauncher.launchServer(
            ...
            println(
                "Executed tasks during initialization : " +
                    taskPaths.joinToString(separator = System.lineSeparator())
            )
            taskPaths.clear()

            val (isSuccessful, failure) =
                server.executeTasks(TaskExecutionMessage(tasks = listOf("assembleDebug"))).get()
